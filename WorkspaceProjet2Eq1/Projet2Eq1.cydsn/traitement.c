...
*
* Paramètres:
*  uint32_t     *signal - Pointeur vers le signal
...
*  uint32_t     *DC     - Pointeur vers la variable de la composante DC du signal
*
*******************************************************************************/
void traitement(uint32_t *signal, int *HR, uint32_t *AC, uint32_t *DC)
{
    ...
    // Nettoyer le signal avec une moyenne mobile sur 8 données (window size)
    int w = 8; 
    ...
    *AC   = 0;
    ...
    for (int i = 0; i < 2000/s; i++)
    {
        ...
        *DC = *DC + (MAX + MIN)/2;
        *AC = *AC + (MAX - MIN); // ################## diviser par 2 enlever vérifier ailleurs
...
***************************************************************************************
* Résumé:
* Fonction d'interprétation des résultats du traitement de signal. Calcul des
* valeurs de fréquence cardiaque et calcul de la saturation selon les composants
* AC et DC extraits des signaux. Gestion des cas limites et/ou hors norme.
*
* Paramètres:
*  int          HR_RED    - Variable de fréquence cardiaque avec le signal rouge
*  int          HR_IR     - Variable de fréquence cardiaque avec le signal infra-rouge
*  uint32_t     AC_RED    - Variable de la composante AC du signal rouge
*  uint32_t     DC_RED    - Variable de la composante DC du signal rouge
*  uint32_t     AC_IR     - Variable de la composante AC du signal infra-rouge
*  uint32_t     DC_IR     - Variable de la composante DC du signal infra-rouge
*  uint32_t     *HR       - Pointeur vers la variable de fréquence cardiaque
*  uint32_t     *HR       - Pointeur vers la variable de la saturation
*
****************************************************************************************/
